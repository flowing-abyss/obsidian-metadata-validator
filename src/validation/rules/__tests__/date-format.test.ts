...
describe("checkDateFormat", () => {
	...
	it("returns null when value matches YYYY-MM-DD format", () => {
		expect(
			checkDateFormat("date", "2024-03-15", "YYYY-MM-DD", PATH),
		).toBeNull();
	});
	...
	it("returns null for DD/MM/YYYY format with matching value", () => {
		expect(
			checkDateFormat("date", "15/03/2024", "DD/MM/YYYY", PATH),
		).toBeNull();
	});
	...
	it("returns null for MM/DD/YYYY format", () => {
		expect(
			checkDateFormat("date", "03/15/2024", "MM/DD/YYYY", PATH),
		).toBeNull();
	});
	...
	it("returns null for Feb 29 in a leap year", () => {
		expect(
			checkDateFormat("date", "2024-02-29", "YYYY-MM-DD", PATH),
		).toBeNull();
	});
	...
	it("returns null for single M/D format", () => {
		expect(checkDateFormat("date", "3/15/2024", "M/D/YYYY", PATH)).toBeNull();
	});

	it("returns error for single M/D format with wrong value", () => {
		const r = checkDateFormat("date", "2024-03-15", "M/D/YYYY", PATH);
		expect(r?.rule).toBe("date-format");
	});
});
