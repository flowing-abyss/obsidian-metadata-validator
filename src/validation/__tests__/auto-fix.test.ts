...
describe("applyAutoFix", () => {
  ...
  it("wraps scalar value in array for list type", () => {
    const field: ManifestField = { type: "list" };
    const fm: Record<string, unknown> = { items: "single" };
    const changed = applyAutoFix("items", field, fm);
    expect(fm["items"]).toEqual(["single"]);
    expect(changed).toBe(true);
  });

  it("does not wrap null value in array for list type", () => {
    const field: ManifestField = { type: "list" };
    const fm: Record<string, unknown> = { items: null };
    const changed = applyAutoFix("items", field, fm);
    expect(fm["items"]).toBeNull();
    expect(changed).toBe(false);
  });
});
